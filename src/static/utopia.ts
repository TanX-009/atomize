...
const mapStepToLabel = (
  step: number,
  labelGroup: UtopiaLabelStyle = "utopia",
) => {
...
export const calculateTypeScale = (config: UtopiaTypeConfig): UtopiaStep[] => {
  ...
  const negativeSteps = Array.from({ length: config.negativeSteps || 0 }).map(
    (_, i) => calculateTypeStep(config, -1 * (i + 1)),
  );
...
export const calculateSpaceScale = (
  config: UtopiaSpaceConfig,
): UtopiaSpaceScale => {
